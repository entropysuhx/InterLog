...
export default function MigrationPrompt() {
  ...
  useEffect(() => {
    ...
    setRecordCount(data.activities.length + data.reflections.length + data.focusSessions.length);
  }, []);
    ...
    <div className="space-y-ds-16">
      <div className="rounded-lg bg-surface-subtle p-ds-16">
        ...
        <p className="mt-ds-8 text-body-sm text-text-secondary">
          Add them to your account? Imports are idempotent and local data is cleared only after success.
        </p>
      </div>
      {message && (
        <p role="status" className={status === "error" ? "text-body-sm text-status-error" : "text-body-sm text-text-secondary"}>
          {message}
        </p>
      )}
      <button
      ...
      >
        {status === "loading" ? <LoaderCircle className="animate-spin" size={16} aria-hidden="true" /> : status === "success" ? <Check size={16} aria-hidden="true" /> : null}
